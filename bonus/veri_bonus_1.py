...
def gnba_to_nba(g):
    nba ={}
    sigma = g['sigma']
    # q = {(s,i) for i in range(1,len(g['f']+1)) for s in g['q']}
    f = {(a,1) for a in g['f'][0]}
    q0 = {(a,1) for a in g['q0']}
    delta = set()
    q=set()
    for (s,sig,stag) in g['delta']:
        for i in range(len(g['f'])):
            if s not in g['f'][i]:
                q.add((s,i+1))
                q.add((stag,i+1))
                delta.add(((s,i+1),sig,(stag,i+1)))
            else:
                q.add((s,i+1))
                q.add((stag,i+1))
                delta.add(((s,i+1),sig,(stag,((i+1)%len(g['f']))+1)))
    nba['sigma']=sigma
    nba['f']=f
    nba['q0']=q0
    nba['delta']=delta
